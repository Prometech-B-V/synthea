...
public abstract class State implements Cloneable, Serializable {
     ...
     * @param person           the person being simulated
     * @param time             the date within the simulated world
     * @param terminateOnDeath whether to terminate immediately and not process the state if the patient is dead
    ...
    public static class WaitUntil extends Delayable {
        ...
        protected void initialize(Module module, String name, JsonObject definition) {
            ...
            if (definition.has("time")) {
                time = Instant.parse(definition.get("time").getAsJsonObject().get("date").getAsString());
            } else {
                throw new IllegalArgumentException("WaitUntil state must have a time");
            }
        }
    ...
    public static class PointOfInterest extends State {
        ...
        protected void initialize(Module module, String name, JsonObject definition) {
            ...
            if (!definition.has("config")) {
                throw new IllegalArgumentException("PointOfInterest state must have a config");
            }

            JsonObject config = definition.getAsJsonObject("config");

            if (!config.has("type")) {
                throw new IllegalArgumentException("PointOfInterest config must have a type");
            }

            if (config.has("contaminated")) {
                this.contaminated = config.get("contaminated").getAsBoolean();
            }
